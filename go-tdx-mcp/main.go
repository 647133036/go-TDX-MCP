...
func runMCP(client *tdx.UnifiedClient) {
	...
	fmt.Fprintf(os.Stderr, "TDX Finance MCP v1.0.6 (stdio): %d 工具 + 45 投资技能\n", toolCount())
...
func runSSE(client *tdx.UnifiedClient, cfg *Config) {
	...
	fmt.Fprintf(os.Stderr, "TDX Finance MCP v1.0.6 (SSE): %d 工具 + 45 投资技能\n", toolCount())
...
func runStreamableHTTP(client *tdx.UnifiedClient, cfg *Config) {
	...
	fmt.Fprintf(os.Stderr, "TDX Finance MCP v1.0.6 (Streamable HTTP): %d 工具 + 45 投资技能\n", toolCount())
...
func runCombined(client *tdx.UnifiedClient, cfg *Config) {
	...
	addr := fmt.Sprintf("0.0.0.0:%d", cfg.WebPort)
	fmt.Fprintf(os.Stderr, "TDX Finance Combined v1.0.6: %d 工具 + 45 投资技能\n", toolCount())
